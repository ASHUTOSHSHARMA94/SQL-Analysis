...
State as [States]
from
[dbo].[FACT_TRANSACTIONS] T1
inner join
[dbo].[DIM_CUSTOMER] T2 on T2.IDCustomer = T1.IDCustomer
inner join
[dbo].[DIM_LOCATION] T3 on T3. IDLocation = T1.IDLocation
where Date >= '2005'
group by
State
...
[dbo].[FACT_TRANSACTIONS] T1
inner join
[dbo].[DIM_LOCATION] T2 on T2.IDLocation = T1.IDLocation
inner join
[dbo].[DIM_MODEL] T3 on T3.IDModel = T1.IDModel
inner join
[dbo].[DIM_MANUFACTURER] T4 on T4.IDManufacturer = T3.IDManufacturer
...
select
top 1
T3.IDModel as [Model ID],
Model_Name as [Model],
ZipCode as [Zip code],
Country as [Country],
State as [State],
IDManufacturer as [Manufacturer],
TotalPrice as [Price]
from
[dbo].[FACT_TRANSACTIONS] T1
inner join
...
IDManufacturer
order by
avg(Unit_price) desc) t1
inner join
[dbo].[FACT_TRANSACTIONS] t2 on t2.IDModel =IDModel
group by
IDModel
order by
avg(TotalPrice) desc
...
[dbo].[DIM_CUSTOMER] T1
inner join
[dbo].[FACT_TRANSACTIONS] T2 on T2.IDCustomer = T1.IDCustomer
inner join
[dbo].[DIM_DATE] T3 on T3.DATE = T2.Date
where YEAR = '2009'
group by
Customer_Name
having
...
select * from
(select top 5
IDModel from [dbo].[FACT_TRANSACTIONS] where year(Date) = '2008'
group by
IDModel
order by
Sum(Quantity) Desc) t1
intersect
select * from
(select top 5
IDModel from [dbo].[FACT_TRANSACTIONS] where year(Date) = '2009'
group by
IDModel
order by
Sum(Quantity) Desc) t2
intersect
select * from
(select top 5
IDModel from [dbo].[FACT_TRANSACTIONS] where year(Date) = '2010'
group by
IDModel
order by
Sum(Quantity) Desc) t3
...
[dbo].[FACT_TRANSACTIONS] T1
inner join
[dbo].[DIM_MODEL] T2 on T2.IDModel = T1.IDModel
inner join
[dbo].[DIM_MANUFACTURER] T3 on T3.IDManufacturer = T2.IDManufacturer 
